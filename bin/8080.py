#!/usr/bin/env python
'''
Intel 8080 Assembler napisan u Python 2.7
Svi sistemi su podrzani.

Autor: Zvonimir Rudinski
Datum: 18.02.2017
'''
from os import path
                    ...
                    if scLine == 'nop':
                        ...
                        try:
                                if scLine.split(',')[1] in variableAddr:
                                    romFile.write(variableAddr[scLine.split(',')[1]][0:4])
                                else:
                            ...
                            try:
                                if scLine.split(',')[1] in variableAddr:
                                    romFile.write(variableAddr[scLine.split(',')[1]][0:1])
                                else:
                        ...
                        try:
                                if scLine.split(',')[1] in variableAddr:
                                    romFile.write(variableAddr[scLine.split(',')[1]][0:2])
                                else:
                                    romFile.write(unhex(scLine.split(',')[1][0:4]))
                        except ValueError:
			                    raise TypeError
